...
- [DRM/EGL](./examples/drm-egl) (`import "github.com/AllenDang/cimgui-go/backend/drmeglbackend"`, Linux only, build with `-tags drmegl`).
...
> [!tip]
> To build the DRM/EGL example/backend, install `libdrm-dev`, `libgbm-dev`, `libegl1-mesa-dev`, `libgles2-mesa-dev`, then use `go run -tags drmegl ./examples/drm-egl`.
